...
function Get-InventoryTotalBytes {
    param(
        [Parameter(Mandatory = $true)]
    ...
    foreach ($record in @($Inventory)) {
        if ($null -eq $record -or
            -not ($record -is [System.Collections.IDictionary]) -or
            -not $record.Contains('bytes')) {
            Throw-A0Failure 'ACQUISITION_OUTPUT_SIZE_INVALID' `
                "Inventory record has no readable byte count."
        }
        [int64]$bytes = $record['bytes']
        if ($bytes -lt 0 -or $totalBytes -gt ([int64]::MaxValue - $bytes)) {
            Throw-A0Failure 'ACQUISITION_OUTPUT_SIZE_INVALID' `
                "Inventory byte count is negative or exceeds Int64."
        }
        $totalBytes += $bytes
    }
    return $totalBytes
...
function Write-TerminalReceipt {
    ...
    $totalBytes = Get-InventoryTotalBytes -Inventory @($inventory)
